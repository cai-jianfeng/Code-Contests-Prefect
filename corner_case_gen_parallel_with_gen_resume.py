...
class CornerCaseGenerator:
    ...
    def generate_for_sample(self, sample: Dict, dataset_type: str = "code_contests_test", max_workers: int = None, results_dir: str = "") -> Tuple[List[Dict], List[Dict]]:
        ...
        if os.path.exists(os.path.join(results_dir, f"{sample_id.replace('/', '_')}.json")):
            
            # 先将 .json 文件复制为 .json.bak 以防止被覆盖
            shutil.move(os.path.join(results_dir, f"{sample_id.replace('/', '_')}.json"), os.path.join(results_dir, f"{sample_id.replace('/', '_')}.json.bak"))
            with open(os.path.join(results_dir, f"{sample_id.replace('/', '_')}.json.bak"), "r") as f:
                existing_results = json.load(f)
                all_results = existing_results.get('result', [])
                begin = len(all_results)
                if begin:
                    log_sample(sample_id, f"Resuming from iteration {begin} with {len(all_results)} previous results")
                    corner_cases = all_results[-1]['corner_cases']
                    messages = all_results[-1]['messages']
                    input_constraints_summary = all_results[0].get('input_constraints_summary', input_constraints_summary)
                    pattern = re.compile(r"Current command list:\s*(.*?)\s*For each command,")
                    all_commands = pattern.findall(messages[-1]['content'])
