    ...
    for attempt in range(max_retries + 1):
        try:
            with SESSION.post(
                ...
                if is_time_limit_exceeded(response):
                    if attempt < max_retries:
                        print(f"TimeLimitExceeded detected for {id} on {api_path}, retrying... (attempt {attempt + 1}/{max_retries})")
                        if retry_delay > 0:
                            time.sleep(retry_delay)
                        continue
                    else:
                        print(f"TimeLimitExceeded persists for {id} on {api_path} after {max_retries} retries, will requeue")
...
if __name__ == "__main__":
    ...
    codecontests_data_path = "/aiarena/group/llmgroup/caijf/Code-Contests-Ours/test_all_solutions_o4mini_10maxsample/"
    ...
    for i in range(sandbox_num):
        ...
        sandbox_api_paths.append(f"http://10.244.81.219:{8080+i}/submit")
    ...
    results_path = f"/aiarena/group/llmgroup/caijf/final_ut_gen/code_contests_ours_all_solutions_o4mini_10maxsample"
    time_consume_path = f"/aiarena/group/llmgroup/caijf/final_ut_gen/code_contests_ours_all_solutions_o4mini_10maxsample.txt"
    # 获取初始时间
