...
def calculate_timeout_multiplier(actual_stdout, expected_stdout):
    ...
    if actual_stdout and expected_stdout.startswith(actual_stdout):
        # 计算完成比例
        completion_ratio = len(actual_stdout) / len(expected_stdout)
        
        # 限制最大倍数避免过长等待
        multiplier = min(completion_ratio, 10)
                ...
                with solution_results_lock:
                    ...
                    result = {
                        ...
                        'progress': {
                            ...
                            'is_complete': (sr['completed_solutions'] == sr['total_solutions'] and 
...
if __name__ == "__main__":
    ...
    results_path = f"/aiarena/group/llmgroup/caijf/final_ut_gen/code_contests_only_generate/"
    time_consume_path = f"/aiarena/group/llmgroup/caijf/final_ut_gen/code_contests_only_generate.txt"
    # 获取初始时间
